...
def validate_string_PDA(input_string, start_state, accept_states, rules):
    queue=deque()
    queue.append((start_state, input_string, []))
    ...
    while queue:
        ...
        rest_input=remaining_input[1:] if remaining_input else ""
        ...
        for (state, symbol, pop_symbol), transitions in rules.items():
            ...
            is_epsilon = (symbol == "")
            matches_input = (symbol == input_symbol)

            if is_epsilon or matches_input:
                if pop_symbol=="" or (stack and stack[-1]==pop_symbol):
                    new_stack=stack[:]
                    new_remaining_input = remaining_input if is_epsilon else rest_input
                    ...
                    for push_symbol, next_state in transitions:
                        ...
                        queue.append((next_state, new_remaining_input, temp_stack))
